...
export const Hero: React.FC = () => {
  return (
    <section id="home" className="relative pt-32 pb-20 lg:pt-48 lg:pb-32 overflow-hidden">
      ...
      <div className="absolute top-0 right-0 w-1/2 h-full bg-gradient-to-l from-coffee-100 to-transparent -z-10 rounded-l-full opacity-60"></div>
      
      {/* Floating beans */}
      <img 
        src="https://images.unsplash.com/photo-1587049352846-4a222e784d38?auto=format&fit=crop&q=80&w=200" 
        alt="Coffee Bean" 
        className="absolute top-40 left-10 w-12 h-12 object-contain opacity-40 rotate-12 blur-sm hidden lg:block"
      />
       <img 
        src="https://images.unsplash.com/photo-1587049352846-4a222e784d38?auto=format&fit=crop&q=80&w=200" 
        alt="Coffee Bean" 
        className="absolute bottom-20 right-1/4 w-16 h-16 object-contain opacity-30 -rotate-45 blur-[2px] hidden lg:block"
      />

      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col-reverse lg:flex-row items-center gap-12">
        {/* Text Content */}
        <div className="flex-1 text-center lg:text-left z-10">
            <div className="inline-block px-4 py-1.5 mb-6 rounded-full bg-coffee-100 text-coffee-800 text-sm font-semibold tracking-wide uppercase">
                Best Coffee In Town
            </div>
            <h1 className="text-5xl lg:text-7xl font-serif font-bold text-coffee-900 leading-[1.1] mb-6">
                Enjoy a new blend of <span className="text-accent italic">coffee beans</span>
            </h1>
            <p className="text-lg text-coffee-700 mb-10 max-w-lg mx-auto lg:mx-0 leading-relaxed">
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center lg:justify-start gap-4">
                <button className="bg-coffee-900 text-white px-8 py-4 rounded-full font-medium flex items-center gap-2 hover:bg-accent transition-all shadow-lg hover:shadow-xl group">
                    Shop Now
                    <span className="bg-accent rounded-full p-1 group-hover:bg-white group-hover:text-accent transition-colors">
                        <ArrowRight className="w-4 h-4" />
                    </span>
                </button>
                <button className="flex items-center gap-3 text-coffee-800 font-semibold px-6 py-4 rounded-full hover:bg-coffee-100 transition-colors">
